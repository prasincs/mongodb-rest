...
var getDB = function(req){
  return new mongo.Db(req.params.db, 
                      new mongo.Server(config.db.host, 
                                       config.db.port, 
                                       {'auto_reconnect': true}));
};

var connectMongo = function(db, callback) {
  db.open(function(err,db) {
    db.authenticate(config.db.username, config.db.password, callback);
  });
};


/**
 * Get request to get all the collections 
 */
app.get('/:db/collections', function(req, res){
  // do validation stuff
  var db = getDB(req);  
  connectMongo(db, function(){
    db.collections(function(err, collections){
      ...
      if (err){
        ...
        collections.forEach (function (collection){
          arr.push(collection.collectionName);
        });
